...
export const DoctorWorkspace = () => {
  ...
  // Initialize PeerJS
  useEffect(() => {
    if (activeTab === "consult" && !peerInstance) {
      const peer = new Peer(`doc_${user.id}`);
      
      peer.on("open", (id) => {
        console.log("Doctor Peer initialized with ID:", id);
      });
    ...
    return () => {
      if (peerInstance && activeTab !== "consult") {
  ...
  const initiateCall = () => {
    navigator.mediaDevices.getUserMedia({ video: true, audio: true })
      .then((stream) => {
        ...
        if (peerInstance && activeApt) {
          const call = peerInstance.call(`pat_${activeApt.patientId}`, stream);
          if (call) {
            call.on("stream", (remoteStream) => {
              if (remoteVideoRef.current) remoteVideoRef.current.srcObject = remoteStream;
            });
